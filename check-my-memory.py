...
if root:
    ...
    cmd = 'dmidecode | egrep -A8 -i -e "Memory Device$" | grep -i size'
    ...
    print("Total of physical memory modules found", dimmtotal, "GB", "in", counter, "memory module(s)")
...
for thisline in os.popen(cmd).readlines():
    ...
    # [    0.000000] kernel: BIOS-e820: [mem 0x0000000000100000-0x0000000069f07fff] usable
    # [    0.000000] kernel: BIOS-e820: [mem 0x0000000071eff000-0x0000000071efffff] usable
    # [    0.000000] kernel: BIOS-e820: [mem 0x0000000100000000-0x000000027fbfffff] usable

    if thisline.find("mem") > -1:
        # something with mem
        hexstuff = thisline.split()[5].rstrip("]")  # Now '0x00000000db71f000-0x00000000db77ffff'
...
if not xen:
    print("BIOS offers", int(biostotal), "MB as usable")
else:
...
print("Memory difference between DIMM hardware and BIOS offering", int(dimmtotal - biostotal), "MB")
print("Memory difference between BIOS offering and memory seen by OS", int(biostotal - freetotal), "MB")
print("Memory difference between DIMM hardware and memory seen by OS", int(dimmtotal - freetotal), "MB")
